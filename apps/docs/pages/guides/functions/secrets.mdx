...
```bash
echo "MY_SECRET_NAME=Yoda" >> ./supabase/.env.local
```
...
Now let's access this environment variable `MY_SECRET_NAME` inside our Function. Anywhere in your function, add this line:

```jsx
console.log(Deno.env.get('MY_SECRET_NAME'))
```
...
supabase secrets set MY_SECRET_NAME=Chewbacca
```
